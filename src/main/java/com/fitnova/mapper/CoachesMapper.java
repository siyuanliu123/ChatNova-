...
public interface CoachesMapper {
    ...
    Coach findById(@Param("coachId") Integer coachId);

    @Update("UPDATE Coaches SET verification_status = #{verificationStatus}, specialty = #{specialty}, " +
            "bio = #{bio}, rating = #{rating} WHERE coach_id = #{coachId}")
